...
class BinaryTree {
    ...
    public:
        ...
        // Copy constructor
        BinaryTree(const BinaryTree<T>& other) : value(other.value), children(other.children), numChildren(other.numChildren){}
        // Destruktor
        ~BinaryTree(){}
        ...
        */
        BinaryTree<T>& addChild(T other){
            if(numChildren >= 2){
                throw "Tidak bisa memiliki lebih dari dua children\n";
            }
            BinaryTree<T> child;
            child.setValue(other);
